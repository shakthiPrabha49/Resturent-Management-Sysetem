...
import { CreditCard, Printer, CheckCircle, Wallet, FileText, Loader2 } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 pb-12">
      <div className="xl:col-span-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2"><CreditCard size={22} className="text-indigo-600" /> Billing Terminal</h2>
            <p className="text-sm text-slate-500 font-medium">Settle completed table orders.</p>
          </div>
          <span className="px-3 py-1 bg-emerald-50 text-emerald-700 rounded-lg text-[10px] font-bold uppercase tracking-wider border border-emerald-100 shadow-sm">{finalizedBills.length} BILLS READY</span>
        </div>

        {finalizedBills.length === 0 ? (
          <div className="bg-white rounded-xl py-24 text-center border border-slate-200 shadow-sm">
            <div className="w-16 h-16 bg-slate-50 rounded-xl flex items-center justify-center mx-auto mb-4"><FileText size={32} className="text-slate-300" /></div>
            <h3 className="text-lg font-bold text-slate-800">No pending bills</h3>
            <p className="text-slate-400 font-medium mt-1">When a table is marked as "Completed", it will appear here.</p>
          </div>
        ) : (
            ...
            {finalizedBills.map(order => (
              <div key={order.id} className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col hover:shadow-md transition-all">
                <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                  ...
                  <button className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-slate-50 rounded-lg transition-all"><Printer size={18} /></button>
                </div>
                ...
                <div className="p-6 flex-1 space-y-2">
                  {order.items.map((item, i) => (
                    <div key={i} className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-bold text-slate-800">Rs. {(Number(item.price) * item.quantity).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="p-6 bg-slate-50/50 border-t border-slate-100">
                  <div className="flex justify-between items-end mb-5">
                    <div>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Subtotal: Rs. {Number(order.total).toFixed(2)}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Tax (5%): Rs. {(Number(order.total) * 0.05).toFixed(2)}</p>
                    </div>
                    <p className="text-2xl font-bold text-indigo-600">Rs. {(Number(order.total) * 1.05).toFixed(2)}</p>
                  </div>
                  <button 
      ...
      <div className="xl:col-span-4 space-y-6">
        <div className="bg-slate-900 rounded-xl p-8 text-white shadow-md">
          ...
          <h3 className="text-4xl font-bold mb-8">Rs. {netBalance.toFixed(2)}</h3>
          <div className="space-y-3 pt-6 border-t border-slate-800">
            <div className="flex justify-between items-center"><span className="text-xs font-semibold text-slate-400">Total Sales</span><span className="text-sm font-bold text-emerald-400">+Rs. {todaysSales.toFixed(2)}</span></div>
            <div className="flex justify-between items-center"><span className="text-xs font-semibold text-slate-400">Expenses</span><span className="text-sm font-bold text-rose-400">-Rs. {todaysExpenses.toFixed(2)}</span></div>
          </div>
        </div>

        <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
          <h3 className="text-md font-bold text-slate-800 flex items-center gap-2 mb-6"><Wallet size={18} className="text-indigo-600" /> Petty Cash Out</h3>
