...
import { ShoppingBag, X, Bell, Loader2, Edit3, Eye, EyeOff, PlusCircle } from 'lucide-react';
  ...
  return (
    <div className="relative">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-lg font-black text-slate-800">Tables in My Section ({visibleTables.length})</h2>
        <button 
          onClick={() => setIsEditingLayout(!isEditingLayout)} 
          className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase transition-all ${isEditingLayout ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-600 border'}`}
        >
          {isEditingLayout ? <Eye size={14} /> : <Edit3 size={14} />}
          {isEditingLayout ? 'Finish Editing' : 'Edit My Layout'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
            ...
            {visibleTables.map(table => (
              <div key={table.id} className="relative group">
                <button 
                  ...
                  className={`w-full p-5 rounded-[28px] border-2 transition-all text-left ${selectedTable?.id === table.id ? 'border-indigo-600 bg-indigo-50' : 'bg-white border-slate-100 hover:border-indigo-200 shadow-sm'}`}
                >
                  ...
                  {table.status !== TableStatus.AVAILABLE && !isEditingLayout && (
                ...
                {isEditingLayout && (
                  <button 
                    ...
                    className="absolute -top-2 -right-2 p-2 bg-rose-500 text-white rounded-full shadow-lg hover:scale-110 transition-transform z-10"
                  >
                    <EyeOff size={12} />
                  </button>
                )}
          ...
          {isEditingLayout && hiddenTables.length > 0 && (
            <div className="bg-slate-100/50 p-6 rounded-[32px] border-2 border-dashed border-slate-200">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Hidden Tables</h3>
              <div className="flex flex-wrap gap-3">
                {hiddenTables.map(table => (
                  <button 
                    key={table.id} 
                    onClick={() => toggleTableVisibility(table.id)}
                    className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl border border-slate-200 text-xs font-bold text-slate-600 hover:border-emerald-500 transition-all"
                  >
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="p-8 border-t">
                <button onClick={submitOrder} disabled={cart.length === 0 || isSubmitting} className="w-full py-5 bg-indigo-600 text-white font-black rounded-[24px]">
                  {isSubmitting ? <Loader2 className="animate-spin mx-auto" /> : 'Send to Kitchen'}
                </button>
              </div>
            </div>
          ) : (
